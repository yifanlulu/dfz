...
QINIU_ENABLE = True if settings.DJANGO_UEDITOR_QINIU else False
...
def UploadFile(request):
    ...
    # 上传后的图片的url
    file_url = urllib.basejoin(USettings.gSettings.MEDIA_URL , OutputPathFormat)
    ...
    if state=="SUCCESS":
        if action=="uploadscrawl":
            ...
            if QINIU_ENABLE:
                import upload_qiniu
                result = upload_qiniu.upload_to_qiniu(file,OutputPathFormat)
...
def get_output_path(request,path_format,path_format_var):
    ...
    if not os.path.exists(OutputPath) and not QINIU_ENABLE:
